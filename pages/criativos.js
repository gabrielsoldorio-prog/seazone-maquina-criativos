    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '22px', marginBottom: '20px' }}>
      ...
      {tipo === 'estatico' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <ImageAnnotator src={midia?.imagemUrl || midia?.imagemFal} alt={`Estático estrutura ${estrutura}`} />

          {/* Versão DALL-E se disponível */}
          {midia?.imagemDalle && midia.imagemDalle !== midia.imagemFal && (
            <div style={{ marginTop: '8px' }}>
              <p style={{ margin: '0 0 6px', color: C.label, fontSize: '11px' }}>Versão DALL-E 3</p>
              <img src={midia.imagemDalle} alt="DALL-E 3" style={{ width: '100%', maxWidth: '480px', borderRadius: '8px', display: 'block' }} />
            </div>
          )}
        ...
        <main style={{ maxWidth: '860px', margin: '0 auto', padding: '28px 20px' }}>
          ...
          {loadingMidias && (
            <div style={{ background: '#0f0f14', border: `1px solid ${C.border}`, borderRadius: '10px', padding: '14px 18px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <Spinner />
              <span style={{ color: C.coral, fontSize: '13px', fontWeight: '500' }}>Gerando imagens e áudios via Fal.ai e ElevenLabs...</span>
            </div>
          )}
          {erroMidias && (
